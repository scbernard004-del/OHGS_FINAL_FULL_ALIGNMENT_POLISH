...
/* =========================================================
   FINAL PRO FIX: dependable theme + compact mobile menu
   This patch must stay at the END of script.js
   ========================================================= */
(function(){
  function setTheme(mode){
    var light = mode === 'light';
    ...
    html.classList.toggle('ohgs-light', light);
    body.classList.toggle('ohgs-light', light);
    html.classList.toggle('light', light);
    body.classList.toggle('light', light);
    body.classList.toggle('light-mode', light);
  ...
  window.setOHGSTheme = setTheme;
  window.toggleTheme = function(){
    var now = document.documentElement.getAttribute('data-theme') || currentTheme();
    setTheme(now === 'light' ? 'dark' : 'light');
  };

  function getMenu(){
    return document.getElementById('ohgsMainMenu') || document.getElementById('navMenu') || document.querySelector('.site-header nav');
  }

  function getMenuBtn(){
    return document.querySelector('.ohgs-menu-toggle, .menu-toggle, [data-menu-toggle]');
  }
  ...
  function closeMenu(){
    ...
    if(menu) menu.classList.remove('ohgs-menu-open');
    if(btn){
      btn.classList.remove('is-open');
      btn.setAttribute('aria-expanded','false');
  ...
  function toggleMenu(e){
    ...
    var btn = getMenuBtn();
    if(!menu || !btn) return;
    var open = !menu.classList.contains('ohgs-menu-open');
    menu.classList.toggle('ohgs-menu-open', open);
  ...
  function bindFinalControls(){
    ...
    document.querySelectorAll('.theme-toggle,[data-theme-toggle],#themeToggle,.theme-btn').forEach(function(btn){
      if(btn.dataset.ohgsThemeFinalBound) return;
      btn.dataset.ohgsThemeFinalBound = 'true';
      btn.removeAttribute('onclick');
      btn.addEventListener('click', function(e){
        e.preventDefault();
        e.stopPropagation();
    ...
    if(menuBtn && !menuBtn.dataset.ohgsMenuFinalBound){
      menuBtn.dataset.ohgsMenuFinalBound = 'true';
      menuBtn.removeAttribute('onclick');
      menuBtn.addEventListener('click', toggleMenu, true);
    }

    if(menu && !menu.dataset.ohgsMenuLinkBound){
      menu.dataset.ohgsMenuLinkBound = 'true';
  ...
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', bindFinalControls);
  }else{
    bindFinalControls();
  }

  window.addEventListener('pageshow', bindFinalControls);
  ...
  document.addEventListener('click', function(e){
    ...
    var btn = getMenuBtn();
    if(!menu || !btn) return;
    if(menu.classList.contains('ohgs-menu-open') && !menu.contains(e.target) && !btn.contains(e.target)){
      closeMenu();
    }
  }, true);
})();
...
/* FINAL MENU ICON JS FIX: binds the actual .ohgs-menu-toggle button */
(function(){
  function menu(){ return document.querySelector('#ohgsMainMenu') || document.querySelector('.site-header nav'); }
  function btn(){ return document.querySelector('.ohgs-menu-toggle') || document.querySelector('#menuToggle') || document.querySelector('.menu-toggle') || document.querySelector('.mobile-menu-button'); }
  function closeMenu(){
    var m=menu(), b=btn();
    document.body.classList.remove('ohgs-menu-open');
    document.body.classList.remove('menu-open');
    if(m){ m.classList.remove('open','is-open','active'); m.setAttribute('aria-hidden','true'); }
  ...
  function openMenu(){
    ...
    if(m){ m.classList.add('open','is-open'); m.setAttribute('aria-hidden','false'); }
    if(b){ b.setAttribute('aria-expanded','true'); }
  }
  function toggle(e){
    if(e){ e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation) e.stopImmediatePropagation(); }
    if(document.body.classList.contains('ohgs-menu-open') || document.body.classList.contains('menu-open')) closeMenu(); else openMenu();
  }
  function bind(){
    var b=btn(), m=menu();
    if(m && !m.id) m.id='ohgsMainMenu';
    if(b){
      b.setAttribute('aria-controls','ohgsMainMenu');
      b.setAttribute('aria-expanded','false');
      if(b.dataset.ohgsRealMenuBound!=='yes'){
        b.dataset.ohgsRealMenuBound='yes';
        b.addEventListener('click', toggle, true);
      }
    }
    document.querySelectorAll('#ohgsMainMenu a,.site-header nav a').forEach(function(a){
      if(a.dataset.ohgsCloseBound==='yes') return;
      a.dataset.ohgsCloseBound='yes';
      a.addEventListener('click', closeMenu);
    });
    closeMenu();
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', bind); else bind();
  window.addEventListener('resize', function(){ if(window.innerWidth>1100) closeMenu(); }, {passive:true});
  document.addEventListener('click', function(e){ var h=document.querySelector('.site-header'); if(h && !h.contains(e.target)) closeMenu(); }, true);
})();


/* OHGS FINAL CUSTOMER PRO JS: reliable mobile menu + light mode */
(function(){
  function q(s){return document.querySelector(s)}
  function qa(s){return Array.prototype.slice.call(document.querySelectorAll(s))}
  function menu(){return q('#ohgsMainMenu') || q('.site-header nav') || q('.site-header .nav-links')}
  function btn(){return q('#ohgsMenuButton') || q('.ohgs-menu-toggle') || q('#menuToggle') || q('.menu-toggle') || q('.mobile-menu-button')}
  function closeMenu(){var m=menu(),b=btn();document.body.classList.remove('ohgs-menu-open','menu-open');if(m){m.classList.remove('open','is-open','active');m.setAttribute('aria-hidden','true')}if(b)b.setAttribute('aria-expanded','false')}
  function toggleMenu(e){if(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation()}var m=menu(),b=btn();var open=!document.body.classList.contains('ohgs-menu-open');document.body.classList.toggle('ohgs-menu-open',open);document.body.classList.toggle('menu-open',open);if(m){m.classList.toggle('open',open);m.classList.toggle('is-open',open);m.setAttribute('aria-hidden',open?'false':'true')}if(b)b.setAttribute('aria-expanded',open?'true':'false')}
  function setTheme(mode){var light=mode==='light';var html=document.documentElement,body=document.body;if(!body)return;html.classList.toggle('light',light);body.classList.toggle('light',light);body.classList.toggle('light-mode',light);html.setAttribute('data-theme',light?'light':'dark');body.setAttribute('data-theme',light?'light':'dark');localStorage.setItem('ohgsTheme',light?'light':'dark');localStorage.setItem('theme',light?'light':'dark')}
  function getTheme(){return localStorage.getItem('ohgsTheme')||localStorage.getItem('theme')||'dark'}
  window.toggleTheme=function(){var cur=document.documentElement.getAttribute('data-theme')||getTheme();setTheme(cur==='light'?'dark':'light')}
  function bind(){
    setTheme(getTheme()==='light'?'light':'dark');
    var b=btn();var m=menu();
    if(m){m.id='ohgsMainMenu';m.setAttribute('aria-hidden','true')}
    if(b){b.id='ohgsMenuButton';b.setAttribute('aria-controls','ohgsMainMenu');b.setAttribute('aria-expanded','false');if(!b.dataset.ohgsFinalMenu){b.dataset.ohgsFinalMenu='1';b.addEventListener('click',toggleMenu,true)}}
    qa('#ohgsMainMenu a,.site-header nav a,.site-header .nav-links a').forEach(function(a){if(!a.dataset.ohgsCloseMenu){a.dataset.ohgsCloseMenu='1';a.addEventListener('click',closeMenu)}});
    qa('.theme-toggle,[data-theme-toggle],#themeToggle,.theme-btn').forEach(function(t){if(!t.dataset.ohgsFinalTheme){t.dataset.ohgsFinalTheme='1';t.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();window.toggleTheme()},true)}});
  }
  function scrollHead(){document.body.classList.toggle('ohgs-scrolled',window.scrollY>8);var h=q('.site-header');if(h)h.classList.toggle('scrolled',window.scrollY>8)}
  function boot(){bind();closeMenu();scrollHead();setTimeout(bind,80);setTimeout(function(){var l=q('#ohgsLoader');if(l){l.classList.add('is-hidden');setTimeout(function(){if(l.parentNode)l.remove()},420)}},1200)}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
  window.addEventListener('scroll',scrollHead,{passive:true});
  window.addEventListener('resize',function(){if(window.innerWidth>1100)closeMenu()},{passive:true});
  document.addEventListener('click',function(e){var h=q('.site-header');if(h&&!h.contains(e.target))closeMenu()},{passive:true});
  window.addEventListener('pageshow',function(){setTheme(getTheme()==='light'?'light':'dark')});
})();


/* OHGS FINAL LOCK JS: stable menu/theme/header */
(function(){
  function qs(s){return document.querySelector(s)}
  function qsa(s){return Array.prototype.slice.call(document.querySelectorAll(s))}
  function setTheme(mode){var l=mode==='light';document.documentElement.classList.toggle('light',l);document.documentElement.setAttribute('data-theme',l?'light':'dark');if(document.body){document.body.classList.toggle('light',l);document.body.classList.toggle('light-mode',l);document.body.setAttribute('data-theme',l?'light':'dark')}try{localStorage.setItem('ohgsTheme',l?'light':'dark');localStorage.setItem('theme',l?'light':'dark')}catch(e){}}
  function getTheme(){try{return localStorage.getItem('ohgsTheme')||localStorage.getItem('theme')||'dark'}catch(e){return 'dark'}}
  window.toggleTheme=function(){var now=document.documentElement.getAttribute('data-theme')||getTheme();setTheme(now==='light'?'dark':'light')}
  function menu(){return qs('#ohgsMainMenu')||qs('.ohgs-main-menu')||qs('.site-header nav')}
  function btn(){return qs('#ohgsMenuButton')||qs('#menuToggle')||qs('.ohgs-menu-toggle')||qs('.menu-toggle')}
  function closeMenu(){var m=menu(),b=btn();document.body&&document.body.classList.remove('menu-open');if(m){m.classList.remove('open','is-open','active','show');m.setAttribute('aria-hidden','true')}if(b){b.setAttribute('aria-expanded','false');b.setAttribute('aria-label','Open menu')}}
  function toggleMenu(e){if(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation()}document.body&&document.body.classList.contains('menu-open')?closeMenu():(document.body&&document.body.classList.add('menu-open'),menu()&&(menu().classList.add('open','is-open'),menu().setAttribute('aria-hidden','false')),btn()&&(btn().setAttribute('aria-expanded','true'),btn().setAttribute('aria-label','Close menu')));return false}
  function bind(){setTheme(getTheme()==='light'?'light':'dark');qsa('.theme-toggle,[data-theme-toggle],#themeToggle,.theme-btn').forEach(function(x){if(x.dataset.ohgsThemeLock)return;x.dataset.ohgsThemeLock=1;x.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();window.toggleTheme();return false},true)});var b=btn(),m=menu();if(m){m.id='ohgsMainMenu';m.classList.add('ohgs-main-menu','nav-links');m.setAttribute('aria-hidden','true')}if(b){b.id='ohgsMenuButton';b.classList.add('ohgs-menu-toggle','menu-toggle');b.setAttribute('type','button');b.setAttribute('aria-controls','ohgsMainMenu');b.setAttribute('aria-expanded','false');if(!b.querySelector('span'))b.innerHTML='<span></span><span></span><span></span>';if(!b.dataset.ohgsMenuLock){b.dataset.ohgsMenuLock=1;b.addEventListener('click',toggleMenu,true);b.addEventListener('touchend',toggleMenu,{capture:true,passive:false})}}qsa('#ohgsMainMenu a,.ohgs-main-menu a,.site-header nav a').forEach(function(a){if(!a.dataset.ohgsCloseLock){a.dataset.ohgsCloseLock=1;a.addEventListener('click',closeMenu)}})}
  function head(){var h=qs('.site-header')||qs('header.site-header');if(h){var s=window.scrollY>8;h.classList.toggle('scrolled',s);document.body&&document.body.classList.toggle('ohgs-scrolled',s)}}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){bind();head();setTimeout(bind,200)});else{bind();head();setTimeout(bind,200)}
  window.addEventListener('scroll',head,{passive:true});window.addEventListener('resize',function(){if(innerWidth>980)closeMenu()},{passive:true});document.addEventListener('click',function(e){var h=qs('.site-header');if(h&&!h.contains(e.target))closeMenu()},true);window.addEventListener('pageshow',bind)
})();
